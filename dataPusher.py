...
class E3OSHelper():
    def __init__(self, configSettings, configDirPath, pointsListFileName):
        ...
        for rowNumber in range(0, pointNameSheet.nrows):
            ...
            if e3osPointName is None or len(e3osPointName) == 0:
                dict['pointName'] = edisonPointName
            else:
                dict['pointName'] = e3osPointName

            dict['pointType'] = pointType

            if len(dict['pointName']) <= 0:
                print "empty pointName at row: ", rowNumber
    ...
    def private_getTableString(self, qualifier, pointsAndTypes):
        ...
        for pointAndType in pointsAndTypes:
            ...
            pointnameToIndexMap[ pointName ] = index
        ...
        return pointNameToIndexMap, tableString

    def getDataInRows(self, qualifier, pointsAndTypes, fromDateString, toDateString):

        pointNameToIndexMap, tableString = self.private_getTableString( qualifier, pointsAndTypes)
        ...
        ''' % (tableString, fromDateString, toDateString)
    ...
    def getPayloads(self, rows ):


    # ============= Transform =======================

    def private_transformRowData(self, rows):
...
class EdgeHelperClass():
    def __init__(self, envType, configSettings, siteSid, stationId, stationName, sendingStationName,
    ...
    def checkPoints(self, pointsAndTypes, edisonPointToTypeMap):
    ...
    def postHistory(self, pointsAndTypes, timestamps, timeStampsAndValues):
        ...
        allPointsAreValid = self.checkPoints(pointsAndTypes, edisonPointToTypeMap)
        ...
        maxTimestampsPerPush = 12
        countOfTimestampsInPayload = 0
    ...
    def private_postSlice(self, points):
        ...
        dateTimeStr = pl["points"][0]['timestamps'][0]
        valStr = pl["points"][0]['values'][0]
        if dateTimeStr == '2018-01-01T00:55:00.000Z' :
            print 'there we go'
        print dateTimeStr, valStr
        '''
        ...
        for k, v in dict.iteritems():
            ...
            if k == 'points':
                ...
                for pointDict in pointArray:
                    print '   ', pointIndex, ":", pointDict['name'], pointDict['pointType'], '#ts:', len(
                        pointDict['timestamps']), '#v:', len(pointDict['values'])
                    pointIndex += 1
            else:
                print k, " : ", v
...
if __name__ == '__main__':
    ...
    if siteToLoad == "BlueWater":
        ...
        # ============ v5DemoSavings on OEDEV =======================
        fromDateString = '2017-09-01T00:00:00.000Z'
        toDateString = '2017-09-01T23:59:55.000Z'
        envType = 'OEDEV'
        ...
        # ============Demo1onPROD=======================
        fromDateString = '2018-01-01T00:00:00.000Z'
        toDateString = '2018-01-01T02:59:55.000Z'
        envType = 'PROD'
    ...
    edgeHelper = EdgeHelperClass(envType, configSettings, stationSid, stationID, stationName, sendingStationName,
                                 lastSuccessTimestampString)
    ...
    rows = e3oshelper.getDataInRows(qualifier, pointsAndTypes, fromDateString, toDateString)
    if len(rows) <= 0:
        logError('no data from sql')


    payloads = self.getPayloads( rows )
