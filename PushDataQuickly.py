...
class EdgeHelperClass():
    ...
    def pointsAreValidCheck(self, pointsAndTypes, edisonPointToTypeMap):
    ...
    def getPushPoint(self, pointIndex, pointName, pointType, timestamps, timeStampsAndValues):

        pushPoint = {}
        pushPoint['name'] = pointName
        pushPoint['pointType'] = pointType
        pushPoint['timestamps'] = []
        pushPoint['values'] = []
        pushPoint['path'] = 'foo/bar'
        ...
        for timestamp in timestamps:
            ...
            if temp is not None:
                ...
                bumpedSliceTime = datetime.strptime(timestamp, '%Y-%m-%dT%H:%M:%S.%fZ')
                bumpedSliceTime = bumpedSliceTime + timedelta(minutes=4)
                bumpedSliceTimeStr = bumpedSliceTime.strftime('%Y-%m-%dT%H:%M:%S.%f')[:-3] + 'Z'

                pushPoint['timestamps'].append(bumpedSliceTimeStr)
                pushPoint['values'].append(val)

        return pushPoint

    def postHistory(self, pointsAndTypes, timestamps, timeStampsAndValues):

        edisonPointsList = self.getEdisonPointsList()
        edisonPointToTypeMap = self.getEdisonPointsToTypeMap(edisonPointsList)

        if not self.pointsAreValidCheck(pointsAndTypes, edisonPointToTypeMap):
        ...
        for timestamp in timestamps:
            ...
            if currentTimeStamp is None:
                currentTimeStamp = datetime.strptime(timestamp, '%Y-%m-%dT%H:%M:%S.%fZ')

            timeOfSlice = datetime.strptime(timestamp, '%Y-%m-%dT%H:%M:%S.%fZ')
            if timeOfSlice > currentTimeStamp + timedelta(hours=12) - timedelta(minutes=5):
                postingAtStr = currentTimeStamp.strftime('%Y-%m-%dT%H:%M:%S.%f')[:-3] + 'Z'
                print 'Pushing chunk: ' + postingAtStr

                self.postChunk( pointsAndTypes, chunkStamps, timeStampsAndValues)
                currentTimeStamp = timeOfSlice
                chunkStamps = []

            chunkStamps.append(timestamp)

        if len(chunkStamps) > 0:
            postingAtStr = currentTimeStamp.strftime('%Y-%m-%dT%H:%M:%S.%f')[:-3] + 'Z'
            print 'Pushing LAST chunk: ' + postingAtStr
            self.postChunk(pointsAndTypes, chunkStamps, timeStampsAndValues)


    def postChunk(self, pointsAndTypes, timestamps, timeStampsAndValues):
        ...
        for pointAndTypeName in pointsAndTypes:
            ...
            pushPoint = self.getPushPoint(pointIndex, pointName, pointType, timestamps, timeStampsAndValues)

            if pushPoint is not None and len(pushPoint['values']) > 0:
                    points.append(pushPoint)
            pointIndex += 1
...
if __name__ == '__main__':
    ...
    if siteToLoad == "BlueWater" :
        ...
        #============ DEMO2 on PROD =======================
        fromDateString = '2018-08-29T00:00:00.000Z'
        toDateString = '2018-09-18T00:00:00.000Z'
        envType = 'PROD'
        ...
        pointsListFileName = 'livepointsDemo2_Fixed.xlsx'
